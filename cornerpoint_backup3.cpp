...
cv::imshow("tmpImg",tmpImg);
                    ...
                    float unitVecNorm = getNorm(unitVecs[1]);
...
//                        continue;
                    ...
                    //继续追踪点
                    for(int k1=1;k1<unitCornerPoints[j].size();k1++)
                    {
                        int num = (k + k1)%unitCornerPoints[j].size();
                        
                        std::array<cv::Point,2> unitVecs2;
                        unitCornerPoints[j][num].getVec(unitVecs2);
                        
                        cv::Point nextVec = getRVec(unitVecs2[0],-vec,unitVecs2[1]);
